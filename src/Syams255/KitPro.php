...
class KitPro extends PluginBase implements Listener {
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $args){
		if(strtolower($cmd->getName()) === "ckit"){
			...
			switch(strtolower($args[0])){
				case "list":
					...
					if ($normalKits !== 'Normal Kits: ')
					{
						$sender -> sendMessage("[CrafterKits] " . $normalKits);
					}
					if ($donatorKits !== 'Donator Kits: ')
					{
						$sender -> sendMessage("[CrafterKits] " . $donatorKits);
					}
						return true;
					break;
				case "":
					...
					{
						$sender->sendMessage("[CrafterKits] Di Console Tidak Di Perbolehkan Menggunakan Command CrafterKits");
						return true;
					}
					$username = $sender->getName();
					if (in_array($username, $this -> players))
					{
						$sender->sendMessage('§l§4[CrafterKits] You need to die before you can pick a new kit!');
						return true;
					}
					else
					{
						$sender->sendMessage("[CrafterKits] Usage: /kit <kit name> or /kit list");
						return true;
					}
					break;
				default:
					if (!$sender instanceof Player)
					{
						$sender->sendMessage("[CrafterKits] Console Tidak Di Perbolehkan Menggunakan Command CrafterKits");
						return true;
					}
					$username = $sender->getName();
					if (in_array($username, $this -> players))
					{
						$sender->sendMessage('§l§a[CrafterKits] You need to die before you can pick a new kit!');
						return true;
					}
					if (isset($this -> kits[strtolower($args[0])]))
					{
						...
						{
							$sender->sendMessage('§l§4You are not a donator!');
							return true;
						}
						else
						{
							$this -> giveKit($kit, $sender);
							$sender->sendMessage('§l§a[CrafterKits] Your kit has been given!');
				...
				if (!$sender instanceof Player)
				{
					$sender->sendMessage("§l§4[CrafterKits] Console Tidak Di perbolehkan Menggunakan Command CrafterKits!! ");
					return true;
				}
				$username = $sender->getName();
				if (in_array($username, $this -> players))
			...
			if(strtolower($cmd->getName()) == "donator"){
				...
				switch(strtolower($args[0])){
					...
					case "rmv":
						...
						break;
					default:
						$sender->sendMessage($this->prefix . "Usage: /donator <add|rmv> <username>");
						return true;
						break;
				}
				}
				else{
					$sender->sendMessage("[CrafterKits] Usage: /donator <add|rmv> <username>");
					return true;
				}
			}
		}
		...
		public function onPlayerDeath(PlayerDeathEvent $event){
			$event->getEntity()->sendMessage("§l§4[CrafterKits] You died and may now pick a new kit!");
